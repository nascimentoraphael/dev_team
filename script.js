document.addEventListener('DOMContentLoaded', function () {
  ...
  async function saveSkills() {
    ...
    const saveButton = document.getElementById('save-skills-btn');
    const originalButtonHTML = saveButton ? saveButton.innerHTML : 'Salvar Habilidades'; // Guardar o HTML original
    ...
    if (!targetUserIdForSave) {
      // Restaurar botão se houver erro antes da requisição
      if (saveButton) {
    ...
    try {
      ...
      if (saveButton) {
        saveButton.disabled = false;
        saveButton.innerHTML = originalButtonHTML; // Restaurar o HTML original
      }
      isSavingSkills = false;
    }
  }
